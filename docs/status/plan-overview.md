...
| **D · Campaign module** | `Campaign` model (targetList + persona + playbook + sequence steps) + CRUD/execute API at `/api/campaigns` with org-scoped ref validation ✅; executor generates per-prospect sequences from **stored knowledge only** (never analyzes; skips non-ready prospects; per-step channel fallback email → first available) ✅; "Outreach" page + campaign detail UI (sequence builder, per-prospect results, live polling) ✅. Note: named "Outreach" in the UI — the legacy `/campaigns` page is ProspectList management. Sending/scheduling of generated sequences is future work (roadmap Phase 3). | ✅ Done |
